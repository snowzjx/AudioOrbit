...
**Document status:** Living product and implementation specification — v0.5.1 shipped (Developer ID, notarized, Sparkle updates); hardening continues
...
- [x] Unit tests (76 deterministic policy tests). Integration and manual hardware tests remain open.
...
- [x] Automatic updates via Sparkle: EdDSA-signed appcast generated by the release workflow, hosted on GitHub Releases, with Check for Updates in the menu-bar context menu.
...
For an audio helper associated with a separate visible application process, the first selected window becomes that route's playback-window anchor. Subsequent focus changes to another window of the same application must not move the established route. Resolve the anchor with a stable public Core Graphics window number where possible. If the anchored window temporarily disappears during a full-screen or Space transition, retain the last committed display instead of falling back to another focused window. Moving the anchored window itself may still move the route after the normal display dwell and hysteresis policy. A confirmed audio-output stop followed by a new start is treated as a playback-session boundary, but the release is verified against the renderer identity: if the anchor window still reports the anchored tab's renderer, the stop was a pause/resume of the same tab and the anchor is kept; only a genuinely new renderer releases the anchor for re-selection.


When the playing tab itself moves, the anchor must follow the media, not the chrome. Safari exposes two usable signals in its window chrome: the active tab's renderer identity (`WebViewProcessID`) in each window's BrowserView identifier, and a per-window audio indicator in the unified field. The renderer identity is authoritative: the window reporting the anchored renderer is the playback window. When the anchor window and another window both report it (the anchor's BrowserView stays stale until its active tab navigates), a freshly created reporting window wins. Only when the renderer is gone (Safari restarted it) does the anchor fall back to the unique media-indicator window, or the freshest media window, after a continuity dwell with transient-Accessibility tolerance. A tab torn off into a brand-new window is therefore followed without any user navigation. During a full-screen transition the anchored window becomes invisible with no replacement window, so the route holds its last committed display and freezes anchor re-pinning until the window returns. Safari restarts its WebKit media helpers during some transitions; the route then migrates to the replacement source process — which may be a WebContent renderer (WebRTC/WebAudio media) or the GPU media helper — preserving the session, anchors and destination instead of stopping and rebuilding. The manual re-anchor action pins the anchor authoritatively until the playback session restarts or the pinned window disappears. Simultaneous playback in several windows stays with the established anchor.
...
5. **About** — version/build, author and copyright, live update status, manual Check for Updates with error surfacing, last-check time. The status menu also exposes the native macOS About panel using the same bundle metadata.
...
The current automated suite contains 76 passing tests. Checked items above mean deterministic coverage exists in this repository; unchecked hardware and failure-injection items remain required even when related behavior has been exercised manually.
...
- [x] Verify signing/notarization/update behavior (Developer ID + hardened runtime + notarized + Sparkle releases through v0.5.1). Permission persistence is documented: TCC resets when the signature changes.
...
- (Resolved) Developer ID + hardened runtime + notarized + Sparkle updates shipped through v0.5.1. The Mac App Store remains out of scope for the first release.
